...
const COLORS = ["#3b82f6","#06b6d4","#10b981","#f59e0b","#ef4444","#8b5cf6","#ec4899",
  ...
  "#facc15","#4ade80","#38bdf8","#c084fc","#fb7185","#fbbf24","#a3e635","#2dd4bf","#e879f9"];

const LabelRight = ({ x, y, width, value }) => (
  <text x={x + width + 8} y={y + 12} fill="var(--text-muted)" fontSize={11}>{value}</text>
);

export default function LeadByIndustry({ data }) {
  if (!data?.length) return <EmptyState />;
  const chartH = Math.max(320, data.length * 36);
  return (
    <div style={wrap}>
      <div style={title}>Leads by Industry</div>
      <div style={sub}>Number of leads per industry type</div>
      <ResponsiveContainer width="100%" height={chartH}>
        <BarChart data={data} layout="vertical" margin={{ top: 0, right: 60, left: 10, bottom: 0 }}>
          ...
          <YAxis type="category" dataKey="industry" width={175} stroke="var(--text-muted)" tick={{ fill: "var(--text)", fontSize: 11 }} />
          <Tooltip
